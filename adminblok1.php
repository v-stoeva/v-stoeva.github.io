...
<html>
    ...
    //Избор на блок
    $sql = "SELECT * FROM `blok table` WHERE id=1";
    ...
    if(isset($_POST['addduty']) AND $_POST['addduty']=="Добави задължение"){
        if(isset($_POST['appnumber']) AND $_POST['appnumber']==0){
            ...
            foreach($appartment as $x=>$appnum){
                $sql1="SELECT * FROM blok1 WHERE id=$appnum";
                $query = mysqli_query($conn,$sql1);
                $olddutys = "";
                    if($query){
                         while ($row = mysqli_fetch_assoc($query)) {
                ...
                $rest = $olddutys + $sum;
                $sql="UPDATE blok1 SET olddutys = $rest WHERE id=$appnum";
                $query = mysqli_query($conn,$sql);
...
//Добавяне на плащания
    if(isset($_POST['addpayment']) AND $_POST['addpayment']=="Добави плащане"){
                ...
                $sql="UPDATE blok1 SET olddutys = $rest WHERE id=$appnum";
                $query = mysqli_query($conn,$sql);
                $sql="UPDATE blok1 SET lastpayment = $sum WHERE id=$appnum";
                $query = mysqli_query($conn,$sql);
                $sql="UPDATE blok1 SET month = $month WHERE id=$appnum";
                $query = mysqli_query($conn,$sql);
                if(mysqli_affected_rows($conn) ==1){
    ...
    if(isset($_POST['submit']) AND $_POST['submit']=="Изпращане"){
        ...
        if($isOK){
            ...
            $sql = "INSERT INTO blok1files (month, sum, name, picture) VALUES (\"$filemonth\",\"$duties\",\"$name\",\"$picture\")";
            $query = mysqli_query($conn,$sql);
            if(mysqli_affected_rows($conn) ==1){
            ...
            $blokmonthdutys = 0;
            $sql = "SELECT * FROM `blok1files` WHERE month=$filemonth";
            $query = mysqli_query($conn,$sql);
                if($query){
                    while ($row = mysqli_fetch_assoc($query)) {
                        $blokmonthdutys = $blokmonthdutys + $row['sum'];}
                        $sql = "UPDATE blok1view SET monthlyduties = $blokmonthdutys WHERE month=$filemonth";
                         $query = mysqli_query($conn,$sql);
            ...
            $sumblok1 = 0;
            $sql = "SELECT * FROM `blok1` WHERE month=$filemonth";            
            $query = mysqli_query($conn,$sql);
            if($query){
                while ($row = mysqli_fetch_assoc($query)) {
                $sumblok1 = $sumblok1 + $row['lastpayment'];}           
        
            $sql="UPDATE blok1view SET availability = $sumblok1 WHERE id=$filemonth";
            $query = mysqli_query($conn,$sql);
                //пресмятане на остатък за месеца            
                    $restformonth = $sumblok1 - $blokmonthdutys;
                    $sql="UPDATE blok1view SET restformonth = $restformonth WHERE id=$filemonth";
                    $query = mysqli_query($conn,$sql); }
                             ...
                             <?php show_alerts($alert); ?>
                                <form method="POST">
                                    <div class="form-group">
                                        ...
                                        <option value="0">Избери всички</option>
					                    <?php echo $appnumber;?>
                                        </select>
                                    </div>
                                    <div class="form-group">
